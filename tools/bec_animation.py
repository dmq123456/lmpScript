...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "1=xx 2=xy 3=xz 4=yx 5=yy 6=yz 7=zx 8=zy 9=zz (default: 1)."
        ),
    )
    parser.add_argument(
        "--element",
...
def extract_bec_columns(
    ...
    bec_col = f"c_outbec[{bec_component}]"
    required = ["x", "y", "z", bec_col]
...
def resolve_color_range(
    ...
    all_values = np.concatenate([np.asarray(f["bec"], dtype=float) for f in frames])
...
def render_frame_rgba(
    ...
    color_label = rf"$Z^*_{{{BEC_COMPONENT_LABELS[bec_component]}}}$ (c_outbec[{bec_component}])"
...
def render_frame_png_bytes(
    ...
    image = Image.fromarray(
        render_frame_rgba(frame_data, bec_component, cmap, vmin, vmax, single_layer)
    )
    buffer = io.BytesIO()
            ...
            local_results.append(
                (
                    ...
                    render_frame_png_bytes(
                        frame_data, bec_component, cmap, vmin, vmax, single_layer
                    ),
                )
            )
...
def main() -> None:
    ...
    if is_root:
        try:
            ...
            frames, scanned_frames = load_selected_frames(
                ...
                bec_component=args.bec_component,
            ...
            if not frames:
                ...
                color_range = resolve_color_range(frames, args.vmin, args.vmax)
    ...
    if is_root:
        ...
        print(
            f"[INFO] BEC component   : c_outbec[{args.bec_component}] "
            f"({BEC_COMPONENT_LABELS[args.bec_component]})"
        )
